    ...
    var body: some View {
        Form {
            ...
            Section("Debug") {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    Text("Notch width  \(settings.notchWidthAdjust >= 0 ? "+" : "")\(Int(settings.notchWidthAdjust)) pt")
                    Slider(value: $settings.notchWidthAdjust, in: -60...20, step: 1)
                }

                VStack(alignment: .leading, spacing: 4) {
                    Text("Notch horizontal offset  \(settings.notchOffsetX >= 0 ? "+" : "")\(Int(settings.notchOffsetX)) pt")
                    Slider(value: $settings.notchOffsetX, in: -20...20, step: 1)
                }

                Text("Turn on the red tint, then drag width/offset until the red matches your physical notch.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
        }
